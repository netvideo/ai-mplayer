...
fn main() {
    println!("Testing MPV audio output...");
    ...
    unsafe {
        ...
        println!("Set mute to no: {}", mute_result);
        
        // 启用音频
        let aid_result = mpv_set_option_string(mpv, CString::new("aid").unwrap().as_ptr(), CString::new("1").unwrap().as_ptr());
        println!("Enable audio track 1: {}", aid_result);
        
        // 设置音频缓冲
        let audio_buffer_result = mpv_set_option_string(mpv, CString::new("audio-buffer").unwrap().as_ptr(), CString::new("0.5").unwrap().as_ptr());
        println!("Set audio buffer to 0.5: {}", audio_buffer_result);
        
        // 设置音频延迟
        let audio_delay_result = mpv_set_option_string(mpv, CString::new("audio-delay").unwrap().as_ptr(), CString::new("0").unwrap().as_ptr());
        println!("Set audio delay to 0: {}", audio_delay_result);
        ...
        println!("Loading media file: f\\2.mkv");
        if mpv_command(mpv, c_args.as_mut_ptr()) < 0 {
            ...
            println!("Has audio: {}, result: {}", has_audio, has_audio_result);
            ...
            // 手动设置音频轨道为1
            println!("Setting audio track to 1...");
            let set_aid = 1;
            ...
            for i in 0..5 {
                ...
                let mut has_audio = false;
                let has_audio_result = mpv_get_property(mpv, CString::new("has-audio").unwrap().as_ptr(), mpv_format_MPV_FORMAT_FLAG, &mut has_audio as *mut _ as *mut _);
                println!("Check {} - Has audio: {}, result: {}", i+1, has_audio, has_audio_result);
                ...
                if !audio_out_ptr.is_null() {
                    ...
                    println!("Check {} - Audio output: {}", i+1, audio_out);
                ...
                if !audio_format_ptr.is_null() {
                    ...
                    println!("Check {} - Audio format: {}", i+1, audio_format);
                ...
                println!("Check {} - Audio sample rate: {}, result: {}", i+1, audio_sample_rate, audio_sample_rate_result);
            }
        }
